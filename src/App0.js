...
const links_crawl = []
let $ = cheerio.load(html);
$('#inhalt > table:nth-child(n) > tbody:nth-child(1) > tr:nth-child(2) > td:nth-child(1) > a:nth-child(1)').each(function (i, e) {
    ...
    rp(CORS_PROXY + link )
      .then(html => {
        ...
        $('#seitennavigation > ul:nth-child(n) > li:nth-child(n) > a:nth-child(n)').each(function (i, e) {
            let test =  "https://www.wiwi.rub.de/" + $(this).attr("href")

        if (test.includes("teaching") || test.includes("lehre") | test.includes("lehrveranstaltung") | test.includes("/lehre/")) {
          links_crawl2.push(test);
          }
        });
